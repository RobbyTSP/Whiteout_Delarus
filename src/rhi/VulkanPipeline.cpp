...
VulkanPipeline::VulkanPipeline(
    ...
    // 1. Shaders
    VkShaderModule vertShaderModule = VulkanShader::createShaderModule(device, vertSpvPath);
    VkShaderModule fragShaderModule = VulkanShader::createShaderModule(device, fragSpvPath);
    ...
    pipelineInfo.renderPass = VK_NULL_HANDLE; // None needed for Dynamic Rendering!
    ...
    // Shaders can now be destroyed
    vkDestroyShaderModule(device, fragShaderModule, nullptr);
    vkDestroyShaderModule(device, vertShaderModule, nullptr);

    std::cout << "[Vulkan] Graphics Pipeline created (Dynamic Rendering enabled)." << std::endl;
}
...
VulkanPipeline::VulkanPipeline(
    ...
    if (externalDescriptorLayout != VK_NULL_HANDLE) {
        ...
        if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &m_descriptorSetLayout) != VK_SUCCESS) {
            throw std::runtime_error("Failed to create descriptor set layout for pipeline!");
        }
        m_ownsDescriptorSetLayout = true;
    }

    // Pipeline Layout with Push Constants & Descriptors
    VkPushConstantRange pushConstantRange{};
    pushConstantRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
    pushConstantRange.offset = 0;
    pushConstantRange.size = pushConstantSize;
    ...
    // Modern Vulkan 1.3 Dynamic Rendering Info
    VkPipelineRenderingCreateInfo renderingCreateInfo{};
    renderingCreateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO;
    renderingCreateInfo.colorAttachmentCount = 1;
    renderingCreateInfo.pColorAttachmentFormats = &colorFormat;
    ...
    std::cout << "[Vulkan] Custom Graphics Pipeline created (Dynamic Rendering enabled)." << std::endl;
}
